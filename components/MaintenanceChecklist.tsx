 ...
 *
 * Renders a treatment plan saved from the AI analysis as a live, interactive
 * checklist. Step completion state is persisted to localStorage keyed by logId
 * so pool professionals can check off steps on-site and pick up where they left off.
 *
 * Used in: app/(dashboard)/pools/[id]/maintenance/page.tsx
 * Access gated to Pool Pro and Pool Team plans.
 */
...
const CHEM_STYLE: Record<ChemType, {
  ...
  acid:     { bg: 'rgba(249,115,22,0.08)',  border: 'rgba(249,115,22,0.25)',  badge: 'rgba(249,115,22,0.15)',  color: '#F97316', label: 'Acid',      icon: '🧪' },
  alkali:   { bg: 'rgba(16,185,129,0.08)',  border: 'rgba(16,185,129,0.25)', badge: 'rgba(16,185,129,0.15)', color: '#10B981', label: 'Alkali',    icon: '🧂' },
  chlorine: { bg: 'rgba(0,111,255,0.07)',   border: 'rgba(0,111,255,0.22)',  badge: 'rgba(0,111,255,0.15)',  color: '#36aaf6', label: 'Chlorine',  icon: '⚡' },
  mineral:  { bg: 'rgba(139,92,246,0.07)',  border: 'rgba(139,92,246,0.22)', badge: 'rgba(139,92,246,0.15)', color: '#8B5CF6', label: 'Mineral',   icon: '💎' },
  other:    { bg: 'rgba(148,163,184,0.07)', border: 'rgba(148,163,184,0.2)', badge: 'rgba(148,163,184,0.12)',color: '#94A3B8', label: 'Treatment', icon: '🔬' },
}

const STATUS_COLORS = {
  safe:     { text: '#10B981', bg: 'rgba(16,185,129,0.12)',  border: 'rgba(16,185,129,0.3)',  label: '✓ Balanced' },
  caution:  { text: '#F59E0B', bg: 'rgba(245,158,11,0.12)',  border: 'rgba(245,158,11,0.3)',  label: '⚡ Needs Attention' },
  critical: { text: '#EF4444', bg: 'rgba(239,68,68,0.12)',   border: 'rgba(239,68,68,0.3)',   label: '✗ Action Required' },
}
...
function StepItem({
  step, index, total, done, onToggle,
}: {
  step: TreatmentStep
  index: number
  total: number
  ...
  return (
    <div
      ...
      style={{
        background: done ? 'rgba(16,185,129,0.06)' : s.bg,
        border: done ? '1px solid rgba(16,185,129,0.2)' : `1px solid ${s.border}`,
        opacity: done ? 0.75 : 1,
      }}
    >
      <div className="flex items-start gap-3">
        ...
        <button
          ...
          style={done
            ? { background: '#10B981', boxShadow: '0 2px 8px rgba(16,185,129,0.35)' }
          ...
          <div className="flex items-start justify-between gap-2 mb-1.5">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span
                ...
                style={{ color: done ? '#10B981' : 'white', textDecoration: done ? 'line-through' : 'none' }}
              >
                {step.chemical}
              </span>
              <span
            ...
            {/* Amount hero */}
            <div
              className="flex-shrink-0 text-right px-2.5 py-1 rounded-xl"
              style={{ background: done ? 'rgba(16,185,129,0.15)' : s.badge }}
            >
              <p className="font-display font-black text-base leading-none" style={{ color: done ? '#10B981' : s.color }}>
                {step.amount}
              </p>
            </div>
          </div>

          <p className="text-xs text-white/50 leading-relaxed">{step.how_to_apply}</p>
        </div>
      </div>
      ...
      {index < total - 1 && (
        <div className="flex items-center gap-2 mt-3 ml-11">
          <div className="w-0.5 h-4 rounded-full" style={{ background: done ? '#10B981' : 'rgba(255,255,255,0.12)' }} />
        </div>
      )}
    </div>
  )
  ...
  const [done, setDone] = useState<boolean[]>(() => {
    ...
    try {
      ...
      if (saved) {
        ...
        // Pad or trim to match current steps length
  ...
  return (
    <div
      ...
      style={{
        ...
        border: allDone ? '1px solid rgba(16,185,129,0.25)' : '1px solid rgba(255,255,255,0.1)',
      }}
    >
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          {notes && !expanded && (
            <p className="text-xs text-white/35 truncate mt-0.5">{notes}</p>
          )}
        </div>
      ...
      {/* Progress bar */}
      <div className="h-0.5 mx-5" style={{ background: 'rgba(255,255,255,0.06)' }}>
        <div
          className="h-full rounded-full transition-all duration-500"
          style={{ width: `${pct}%`, background: allDone ? '#10B981' : sc.text }}
        />
      </div>
      ...
      {expanded && (
        <div className="p-5 pt-4 space-y-2">
          ...
          {notes && (
            ...
            >
              <p className="text-[10px] font-bold text-white/25 uppercase tracking-widest mb-1">Service Notes</p>
              <p className="text-sm text-white/55 leading-relaxed">{notes}</p>
            </div>
          )}
          ...
          {allDone && (
            ...
            >
              ...
              <p className="text-sm font-bold text-emerald-400">Treatment complete!</p>
              <p className="text-xs text-white/40 mt-1">All steps checked off. Retest in 24–48 hours to verify results.</p>
            </div>
          )}
            ...
            <div className="pt-2">
              {showDeleteConfirm ? (
                ...
                <button
                  ...
                  className="w-full py-2 text-xs text-white/20 hover:text-red-400 transition-colors"
                >

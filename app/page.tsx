  ...
  return (
    <div className="min-h-screen bg-white" style={{ fontFeatureSettings: '"ss01" on, "tnum" on' }}>
      ...
      <nav className="fixed top-0 inset-x-0 z-50 bg-white" style={{ borderBottom: `1px solid ${FROST}` }}>
        <div className="max-w-[1320px] mx-auto px-6 flex items-center justify-between" style={{ height: '76px' }}>
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-[17px] font-semibold tracking-tight" style={{ color: INK }}>HydroSource AI</span>
          </div>
          <div className="flex items-center gap-6">
            ...
            <Link href="/login" className="text-sm font-normal transition-colors" style={{ color: INK }}>
              Sign in
            </Link>
            <Link href="/signup"
      ...
      <section className="px-6" style={{ paddingTop: '160px', paddingBottom: '96px' }}>
        <div className="max-w-[1320px] mx-auto grid lg:grid-cols-[1fr_440px] gap-16 items-start">

          {/* Left — copy */}
          <div>
            <p className="text-xs font-normal uppercase mb-6" style={{ color: SLATE, letterSpacing: '0.06em' }}>
              Now in beta
            </p>
          ...
          {/* Right — sample analysis panel */}
          <div className="hidden lg:block">
            <div className="p-5 bg-white" style={{ border: `1px solid ${FROST}`, borderRadius: '8px' }}>
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="text-right">
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1 text-[10px] font-medium"
                    style={{ background: 'rgba(217,119,6,0.08)', border: '1px solid rgba(217,119,6,0.25)', color: '#b45309', borderRadius: '4px' }}>
                    Needs attention
                  </span>
              ...
              <div className="space-y-2 mb-4">
                {[
                  ...
                  <div key={r.label} className="px-3 py-2.5" style={{ background: MIST, border: `1px solid ${FROST}`, borderRadius: '4px' }}>
            ...
            <p className="text-xs mt-3 text-center" style={{ color: SLATE }}>Instant diagnosis — no pool store trip</p>
          </div>
        </div>
      </section>

      <div style={{ borderTop: `1px solid ${FROST}` }} />

      {/* ── Features ─────────────────────────────────────────────── */}
      <section className="px-6" style={{ paddingTop: '96px', paddingBottom: '96px' }}>
        <div className="max-w-[1320px] mx-auto">
          <p className="text-xs uppercase mb-4" style={{ color: SLATE, letterSpacing: '0.06em' }}>Everything you need</p>
          <h2 className="font-light mb-16 max-w-xl" style={{ fontSize: 'clamp(2rem, 5vw, 3rem)', lineHeight: 1.1, letterSpacing: '-0.96px', color: INK }}>
            ...
            {features.map((f, i) => (
              <div key={i}>
                ...
                <ul className="space-y-1.5">
                  {f.bullets.map((b) => (
                    <li key={b} className="text-sm leading-relaxed" style={{ color: STEEL }}>
                      {b}
                    </li>
                  ))}
                </ul>
      ...
      {/* ── How it works ─────────────────────────────────────────── */}
      <section className="px-6" style={{ paddingTop: '96px', paddingBottom: '96px', background: MIST }}>
        <div className="max-w-[1320px] mx-auto">
          <p className="text-xs uppercase mb-4" style={{ color: SLATE, letterSpacing: '0.06em' }}>Dead simple</p>
          <h2 className="font-light mb-16 max-w-xl" style={{ fontSize: 'clamp(2rem, 5vw, 3rem)', lineHeight: 1.1, letterSpacing: '-0.96px', color: INK }}>
      ...
      <section className="px-6" style={{ paddingTop: '96px', paddingBottom: '96px' }}>
        <div className="max-w-[720px] mx-auto">
          <p className="text-xs uppercase mb-4" style={{ color: SLATE, letterSpacing: '0.06em' }}>Pricing</p>
          <h2 className="font-light mb-4" style={{ fontSize: 'clamp(2rem, 5vw, 3rem)', lineHeight: 1.1, letterSpacing: '-0.96px', color: INK }}>
            Simple, honest pricing
          </h2>
          <p className="mb-10" style={{ fontSize: '20px', lineHeight: 1.4, color: SLATE, fontWeight: 300 }}>
      ...
      {/* ── Final CTA ─────────────────────────────────────────────── */}
      <section className="px-6 text-center" style={{ paddingTop: '96px', paddingBottom: '96px', background: MIST }}>
        <div className="max-w-[640px] mx-auto">
          <p className="text-xs uppercase mb-4" style={{ color: SLATE, letterSpacing: '0.06em' }}>Now in early beta</p>
          <h2 className="font-light mb-5" style={{ fontSize: 'clamp(2rem, 5vw, 3rem)', lineHeight: 1.1, letterSpacing: '-0.96px', color: INK }}>
            Ready for perfectly balanced water?
          </h2>
          <p className="mb-8" style={{ fontSize: '20px', lineHeight: 1.4, color: SLATE, fontWeight: 300 }}>
